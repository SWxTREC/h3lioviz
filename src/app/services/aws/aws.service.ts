...
export class AwsService {
    ...
    getEc2Status() {
      ...
      //     //   '<p>The Visualizer EC2 instance is currently shutdown. Please press the start button to startup the EC2 instance.
      //     //   Once the EC2 instance is running you will be redirected to the visualizer page.
      //     //   </p><button class='btn default' onClick='startEc2()'>Start EC2</button>';
      //       } else if (request.status === 200 && eC2state === 'pending') {
      //     // document.getElementById('status').innerHTML =
      //     // '<p>The EC2 instance is currently starting, please wait, and you will soon be redirected to the visualizer page.
      //     // This process may take a few minutes.</p>';
      //       } else if (request.status === 200 && eC2state === 'running' && eC2status === 'initializing') {
      //     // document.getElementById('status').innerHTML =
      //     // '<p>The EC2 instance is now running, please wait while the visualizer application is started.
      //     // This process takes around 3 minutes.</p>';
      //       } else if (request.status === 200 && eC2state === 'running' && eC2status === 'ok') {
      //     // document.getElementById('status').innerHTML =
      //     // '<p>The EC2 instance is now running and you will be soon be redirected to the visualizer page.
      //     // (For now please just click the link:)
      //     // <a href='http://paraview.swx-trec.com:8080' target='_blank' rel='noopener noreferrer'>
      //     // Enlil Visualizer </a> <br><br> If you are no longer utilizing the visualizer,
      //    // please stop the EC2 instance: <button class='btn default' onClick='stopEC2()'>Stop EC2</button></p>';
      //       } else if (request.status === 200 && (eC2state === 'stopping' || eC2state === 'shutting-down')) {
      //     // document.getElementById('status').innerHTML =
      //     // '<p>The EC2 instance is currently shutting down. If you need to utilize the visualizer,
      //     // please wait a moment and you will have the opportunity to restart the EC2 instance.
      //     // This process may take a few minutes.</p>';
      //       } else {
      //           console.log(`error ${request.status} ${request.statusText}`);
      //       }
